...
requestRouter.post('/request/process/:status/:requestId', userAuth, async (req, res) => {
  try{
    const { status, requestId } = req.params;

    if(!['accepted', 'rejected'].includes(status)) { // status validation
    ...
    if(!connectionData) { // request validation
